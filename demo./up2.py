...
class NSLookup:
    ...
    def setup_table_ui(self, frame, env):
        ...
        table = ttk.Treeview(frame, columns=("Domain", "Name", "IP Address", "Hanger", "Timestamp"), show="headings")
...
class HTTPRequest:
    ...
    def setup_table_ui(self, frame, env):
        ...
        url_entry.grid(row=1, column=0, padx=2, pady=2)
        url_entry.insert(0, "Enter URL")

        regex_entry = ttk.Entry(frame, width=15)
        regex_entry.grid(row=1, column=1, padx=2, pady=2)
        regex_entry.insert(0, "Enter regex")

        port_entry = ttk.Entry(frame, width=5)
        port_entry.grid(row=1, column=2, padx=2, pady=2)
        ...
        ssl_checkbox.grid(row=1, column=3, padx=2, pady=2)
    ...
    async def fetch_url(self, url, regex, port, use_ssl, cert_path):
    ...
    async def update_http_table_async(self, env):
        ...
        tasks = [self.fetch_url(entry["url"], entry["regex"], entry["port"], entry["use_ssl"], cert_path) if entry["use_ssl"] else self.fetch_url_nossl(entry["url"], entry["regex"], entry["port"]) for entry in self.urls[env]]
        results = await asyncio.gather(*tasks)
        for result in results:
            table.insert("", "end", values=result)
        refresh_time = int(getattr(self, f"{env}_refresh_time_entry").get()) * 1000  # Convert seconds to milliseconds
