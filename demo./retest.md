...
def is_connected_to_vpn():
    ...
    time.sleep(1)  # Wait for 2 seconds to check if counters are increasing
...
class NSLookup:
    ...
    def resolve_domain(self, domain):
        ...
        try:
            ...
            #print(f"Async DNS resolution failed for {domain}: {e}")
            # Fallback to synchronous dns.resolver
            try:
                ...
                resolve_time = (datetime.now() - start_time).total_seconds()
                return (domain, "Error", str(e), "Unknown", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), resolve_time)
...
# Add VPN indicator
    def update_vpn_status():
        ...
        bulb_image = Image.open(f"{bulb_color}_bulb.png")  # Ensure you have green_bulb.png and blue_bulb.png images
        ...
        root.after(600000, update_vpn_status)  # Schedule to run every 10 minutes
