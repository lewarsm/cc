...
def update_gui(q, treeview1, treeview2, welcome_label, results_frame):
    try:
        while True:
        ...
        if treeview1.get_children() and treeview2.get_children():
            # Remove the welcome message and show the results frame
    ...
    root.after(100, update_gui, q, treeview1, treeview2, welcome_label, results_frame)
...
# Create a label for the "Welcome Home" message
welcome_label = ttk.Label(root, text="Welcome Home", font=("Helvetica", 16), padding=10)
welcome_label.pack(pady=50)  # Add padding to center the label

# Create a frame to hold the Treeview widgets (for when results are ready)
results_frame = ttk.Frame(root)

# Create two Treeview widgets for the two tables
frame1 = ttk.Frame(results_frame)
...
frame2 = ttk.Frame(results_frame)
