...
class NSLookup:
    ...
    def load_domains(self, filename, env):
        if not os.path.exists(filename):
            if env == "production":
                initial_data = ["www.google.com", "www.yahoo.com"]
            elif env == "qa":
                initial_data = ["mail.yahoo.com", "www.msn.com"]
            elif env == "development":
                initial_data = ["www.github.com", "www.w3schools.com"]
    ...
    def reset_domains(self, env):
        if env == "production":
            self.domains[env] = ["www.google.com", "www.yahoo.com"]
        elif env == "qa":
            self.domains[env] = ["mail.yahoo.com", "www.msn.com"]
        elif env == "development":
            self.domains[env] = ["www.github.com", "www.w3schools.com"]
        self.save_domains(env)
        self.update_nslookup_table(env)
    ...
    async def resolve_domain(self, domain):
        ...
        try:
            ...
            resolve_time = (datetime.now() - start_time).total_seconds()
            return (domain, "Error", str(e), "", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), resolve_time)

    async def update_nslookup_table_async(self, env):
        table = getattr(self, f"{env}_table")
        table.delete(*table.get_children())  # Clear the table
        tasks = [self.resolve_domain(domain) for domain in self.domains[env]]
        results = await asyncio.gather(*tasks)
        ...
        self.master.after(refresh_time, lambda: asyncio.run(self.update_nslookup_table_async(env)))  # Auto-refresh based on user input

    def update_nslookup_table(self, env):
        asyncio.run(self.update_nslookup_table_async(env))
...
class HTTPRequest:
    ...
    def load_urls(self, filename, env):
        if not os.path.exists(filename):
            if env == "production":
                ...
                initial_data = [
                    {"url": "mail.yahoo.com", "regex": "OK", "port": 443, "use_ssl": True},
                    {"url": "www.msn.com", "regex": "OK", "port": 443, "use_ssl": True}
                ]
            elif env == "development":
                initial_data = [
                    {"url": "www.github.com", "regex": "OK", "port": 443, "use_ssl": True},
                    {"url": "www.w3schools.com", "regex": "OK", "port": 443, "use_ssl": True}
                ]
            with open(filename, "w") as file:
                json.dump(initial_data, file)
        with open(filename, "r") as file:
    ...
    def delete_row(self, event, env):
        selected_item = self.frames[env].table.selection()
        if selected_item:
            for item in selected_item:
                values = self.frames[env].table.item(item, "values")
                url = values[0]
                self.frames[env].table.delete(item)
                self.delete_url(env, url)

    def delete_url(self, env, url):
        self.urls[env] = [entry for entry in self.urls[env] if entry["url"] != url]
        self.save_urls(env)
    ...
    async def fetch_url(self, url, regex, port, use_ssl, cert_path, env):
        ...
        try:  # just check url name on certificate
            ...
            # Check if the hostname matches the CN, is in the SAN list, or matches a wildcard certificate
    ...
    async def update_http_table_async(self, env):
        table = getattr(self, f"{env}_table")
        table.delete(*table.get_children())  # Clear the table
        ssl_context = create_combined_ssl_context(CA_path, cert_path) if cert_path else None

        tasks = [self.fetch_url(entry["url"], entry["regex"], entry["port"], entry["use_ssl"], cert_path, env) if entry["use_ssl"] else self.fetch_url_nossl(entry["url"], entry["regex"], entry["port"]) for entry in self.urls[env]]
        results = await asyncio.gather(*tasks)
        for result in results:
            table.insert("", "end", values=result)
        refresh_time = int(getattr(self, f"{env}_refresh_time_entry").get()) * 1000  # Convert seconds to milliseconds
        self.master.after(refresh_time, lambda: asyncio.run(self.update_http_table_async(env)))  # Auto-refresh based on user input

    def update_http_table(self, env):
        asyncio.run(self.update_http_table_async(env))
   ...
   # ttk.Button(sidebar, text="Create Custom Hosts File", command=lambda: open_hosts_file_window(theme_var.get())).grid(row=13, column=0, padx=5, pady=2)
    ...
    def display_message():
        try:
            ...
            random_message = random.choice(sayings).strip()
        except Exception as e:
                random_message = "Please wait for first update, thanks for your patience."

        # Create a Toplevel window
        message_window = Toplevel(root)
        message_window.title("Message of the Day")
        message_window.geometry("400x200")
        Label(message_window, text=random_message, wraplength=350, padx=10, pady=10).pack(expand=True)
    ...
    def initialize_tools():
        ...
        def run_nslookup():
            try:
                ...
                thread_message_queue.put("NSLookup initialized successfully")
            except Exception as e:
                thread_message_queue.put(f"NSLookup startup failed: {e}")
                log_error("NSLookup startup failed", e)

        def run_http_request():
            try:
                http_request = HTTPRequest(scrollable_frame, theme_var.get())
                thread_message_queue.put("HTTPRequest initialized successfully")
            except Exception as e:
                thread_message_queue.put(f"HTTPRequest startup failed: {e}")
                log_error("HTTPRequest startup failed", e)

        nslookup_thread = threading.Thread(target=run_nslookup)
        http_request_thread = threading.Thread(target=run_http_request)

        nslookup_thread.start()
        http_request_thread.start()

        nslookup_thread.join()
        http_request_thread.join()

        while not thread_message_queue.empty():
            thread_message = thread_message_queue.get()
